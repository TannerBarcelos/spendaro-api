...
export const budgets = pgTable('budgets', {
  id: serial('id').primaryKey(),
  user_id: integer('user_id').references(() => users.id, { onDelete: 'cascade' }), // when a user is deleted, all their budgets should be deleted as well
...
export const budget_categories = pgTable('budget_categories', {
  id: serial('id').primaryKey(),
  budget_id: integer('budget_id').references(() => budgets.id, { onDelete: 'cascade' }), // when a budget is deleted, all its categories should be deleted as well
...
export const budgetCategoriesRelations = relations(budget_categories, ({ one, many }) => ({
  budget: one(budgets),
...
export const budget_category_items = pgTable('budget_category_items', {
  id: serial('id').primaryKey(),
  category_id: integer('category_id').references(() => budget_categories.id, { onDelete: 'cascade' }), // when a category is deleted, all its items should be deleted as well
...
export const budgetCategoryItemsRelations = relations(budget_category_items, ({ one, many }) => ({
...
export const budget_category_item_transactions = pgTable('budget_category_item_transactions', {
  id: serial('id').primaryKey(),
  item_id: integer('item_id').references(() => budget_category_items.id, { onDelete: 'cascade' }), // when an item is deleted, all its transactions should be deleted as well
  ...
  transaction_type_id: integer('transaction_type_id').references(() => budget_category_item_transaction_types.id, { onDelete: 'set null' }), // when a transaction type is deleted, we should set the transaction type id to null since transactions should still persist
  created_at: createdTs,
  updated_at: updatedTs,
});

export const budgetCategoryItemTransactionsRelations = relations(budget_category_item_transactions, ({ one }) => ({
